...
class PartitionClustering():
	...
	def cluster(self, ligands, distanceMatrix, numClusters, verbose=False):
		...
		while centroidUpdate != 0 and iterations < 100:
			...
			if verbose:
				print('Iteration number %d. Sum of changed centroid bits = %d' % (iterations, centroidUpdate))
...
def trialJaccard(hcclusters, pcclusters):
	f11, f01, f10 = 0, 0, 0

	# Determine which ligand pairs are the same in both clusterings, or are only paired in hierarchical clustering
	for hccluster in hcclusters:
		for l1 in range(len(hccluster.ligands)):
			print('looking at hc ligand %d' % (f11+f01))
			for l2 in range(len(hccluster.ligands)):
				if l1 != l2:
					for pccluster in pcclusters:
						if hccluster.ligands[l1] in pccluster.ligands:
							if hccluster.ligands[l2] in pccluster.ligands:
								f11 += 1
							else:
								f01 += 1

	# Determine which ligand pairs are only paired in parititional clustering
	# Only consider the pair to be in the pcclusters and not in the hcclusters if both ligands are
	#  present in the hcclusters (since hcclusters was run on a subset of the data)
	for pccluster in pcclusters:
		for l1 in range(len(pccluster.ligands)):
			print('now looking at pc ligand %d' % (f11+f01+l1+1))
			for l2 in range(len(pccluster.ligands)):
				if l1 != l2:
					for hccluster in hcclusters:
						if pccluster.ligands[l1] in hccluster.ligands:
							if pccluster.ligands[l2] in hccluster.ligands:
								continue
							else:
								for hccluster2 in hcclusters:
									if pccluster.ligands[l2] in hccluster2.ligands:
										f10 += 1

	# The code above counts each pair twice, so divide each of these values by 2
	f11 /= 2
	f01 /= 2
	f10 /= 2
...
"""
def test_silhouette_coeff(distanceMatrix):
